...
<?php
            ...
            // Ambil dari input user (GET) atau default ke bulan ini
            $currentMonth = isset($_GET['bulan']) ? (int) $_GET['bulan'] : (int) date('m');
            $currentYear = isset($_GET['tahun']) ? (int) $_GET['tahun'] : (int) date('Y');
?>
<div class="container mt-4">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div id="hasil-statistik" class="mt-4"></div>
        </div>
    </div>
</div>

<script>
    document.querySelector('#statistik-form').addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            const res = await fetch(`modules/statistik/statistik_bulanan.php?bulan=${bulan}&tahun=${tahun}`);

            const contentType = res.headers.get("content-type");

            if (!contentType || !contentType.includes("application/json")) {
                throw new Error("Bukan JSON: " + await res.text());
            }
            ...
            const total = data.pinjam + data.kembali + data.terlambat;
            ...
            if (total > 0) {
                ...
                <table class="table table-bordered">
                    <thead class="table-light">
                        <tr><th>Jenis Statistik</th><th class="text-center">Jumlah</th></tr>
                    </thead>
                    <tbody>
                        <tr><td>Buku Dipinjam</td><td class="text-center">${data.pinjam}</td></tr>
                        <tr><td>Buku Dikembalikan</td><td class="text-center">${data.kembali}</td></tr>
                        <tr><td>Buku Terlambat</td><td class="text-center">${data.terlambat}</td></tr>
                    </tbody>
                </table>`;
            } else {
                html = `<div class="alert alert-warning text-center">Tidak ada data pada bulan tersebut.</div>`;
            }

            document.getElementById('hasil-statistik').innerHTML = html;
